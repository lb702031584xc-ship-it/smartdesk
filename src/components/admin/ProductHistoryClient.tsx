...
import {
  loadProductRevisionDetailAction,
...
export function ProductHistoryClient({
  ...
  async function openRevision(revisionId: string) {
    ...
    const loaded = await loadProductRevisionDetailAction(productId, revisionId);
    if ("error" in loaded) {
  ...
  async function confirmRestore() {
    ...
    const result = await restoreProductRevisionAction(productId, selectedId, currentVersion);
    if (result.ok) {
      router.push(`/admin/products/${productId}?restored=1`);
      router.refresh();
      return;
    }
  ...
  return (
    <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
      ...
      <section className="rounded-lg border border-[var(--line)] bg-white p-4">
        ...
        {detail.status === "ready" ? (
          <div>
            ...
            <div className="mt-4">
              <p className="text-sm font-medium text-[var(--ink)]">Snapshot summary</p>
              <dl className="mt-2 grid gap-2 text-sm">
                <div>
                  <dt className="text-[var(--subtle)]">Name</dt>
                  <dd>{detail.data.identity.name}</dd>
                </div>
                <div>
                  <dt className="text-[var(--subtle)]">Category</dt>
                  <dd>{detail.data.identity.category}</dd>
                </div>
                <div>
                  <dt className="text-[var(--subtle)]">Rating</dt>
                  <dd>{detail.data.review?.rating ?? "—"}</dd>
                </div>
              </dl>
            </div>
      ...
      <p className="lg:col-span-2 text-sm">
        <Link href={`/admin/products/${productId}`} className="underline-offset-2 hover:underline">
          ← Back to editor
        </Link>
      </p>
    </div>
